...
# ==============================================================================
"""Tests for MobileNetV2 application."""
...
class MobileNetV2Test(test.TestCase):
  ...
  def test_no_top(self):
    model = keras.applications.MobileNetV2(weights=None, include_top=False)
  ...
  def test_with_pooling(self):
    model = keras.applications.MobileNetV2(weights=None,
                                           include_top=False,
                                           pooling='avg')
    self.assertEqual(model.output_shape, (None, 1280))

  def test_weight_loading(self):
    with self.assertRaises(ValueError):
      keras.applications.MobileNetV2(weights='unknown',
                                     include_top=False)
    with self.assertRaises(ValueError):
  ...
  def test_preprocess_input(self):
    ...
    out1 = keras.applications.mobilenet_v2.preprocess_input(x)
  ...
  def test_variable_input_channels(self):
  ...
  def test_image_size(self):
    with self.test_session():
      valid_image_sizes = [96, 128, 160, 192, 224]
      ...
      with self.assertRaises(ValueError):
        model = keras.applications.MobileNetV2(input_shape=invalid_image_shape,
                                             weights='imagenet',
                                             include_top=True)
